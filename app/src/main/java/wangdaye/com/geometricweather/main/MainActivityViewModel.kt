...
class MainActivityViewModel @Inject constructor(
    ...
    fun init(formattedId: String? = null) {
        ...
        val id = formattedId ?: savedStateHandle[KEY_FORMATTED_ID]

        initCompleted = false

        repository.initLocations(
            context = application,
            formattedId = id ?: "",
        ) { totalList, _ ->
            if (totalList == null) return@initLocations
            ...
            validLocationList.value = SelectableLocationList(locationList = validList, selectedId = finalId)
            totalLocationList.value = SelectableLocationList(locationList = totalList, selectedId = finalId)

            loading.setValue(false)
            indicator.setValue(
                Indicator(
                    total = validList.size,
                    index = validList.indexOfFirst { it.formattedId == finalId }
                )
            )
