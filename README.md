...
- 界面：详情卡改成两列量规小卡、每日/小时概览锁到当前时间、首页重排——温度块移到左上角，预警与分钟级降水各成一张卡挂在其下
- 界面文案：补齐简繁中文；境外源的天况文案不再读出原文（Open-Meteo / WeatherAPI / MET Norway 三家文案表中文化）
- 刷新：每次打开 app 自动刷新一次；滑动切换地点仍按刷新率门槛
...
- 日月升落卡在「当天没有月出或月落」的日子会隐藏月亮半边（每个朔望月约各一次，是天文实况而非数据缺失）
- Météo-France 的天气描述是源方原文（法语），没有代码可供映射（仅影响法国地点）
